...
class IngestPipeline:
    ...
    def ingest_file(self, *,delivery_id: str, file_id: str, filename: str) -> None:
        ...
        # Idempotency: if final exists and validates, we are done.
        if self._final_is_good(paths.final_path):
            self._mark(filename, ManifestStatus.MOVED, final_size=str(paths.final_path.stat().st_size), message="Final exists and is valid",)
            logger.info(f"Final already exists and is valid: {filename}")
            return
        
        # If staging exists and validates, just move to final.
        if self._filtered_is_good(paths.filtered_path):
            self._move_filtered_to_final(paths)
            return
    ...
    def _ensure_raw_download(self, paths: ArtifactPaths, *, delivery_id: str, file_id: str) -> bool:
        """Ensure raw archive exists locally and appears complete."""
        ...
        is_complete = downloader.is_local_file_complete(
            self.session, paths.raw_path, url, headers=self._cfg.download.headers
        )
        
        if is_complete:
            self._mark(paths.filename, ManifestStatus.DOWNLOADED, 
                       raw_size=str(paths.raw_path.stat().st_size), 
                       message="Raw download already complete",)
            return True
        ...
        try:
            ...
            self._mark(paths.filename, ManifestStatus.FAILED, message=f"Download failed: {exc}")
            logger.error(f"Download failed for {paths.filename}: {exc}")
            return False
        
        self._mark(paths.filename, ManifestStatus.DOWNLOADED, raw_size=str(paths.raw_path.stat().st_size), message="Download successful",)
        return True
    ...
    def _filter_raw_to_staging(self, paths: ArtifactPaths) -> bool:
        ...
        try:
            ...
            self._mark(paths.filename, ManifestStatus.FAILED, raw_size=str(paths.raw_path.stat().st_size), message=f"Filtering failed: {exc}",)
            logger.error(f"Filtering failed for {paths.filename}: {exc}")
            return False
